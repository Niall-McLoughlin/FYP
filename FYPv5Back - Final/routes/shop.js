...
let nextBarcodeId = 0;
router.post('/addBarcode', (req, res, next) => {
  new Barcode({ ourId: '' + nextBarcodeId, name: req.body.title, amount: req.body.amount, date: req.body.date, barcode: req.body.barcode })
    .save()
    .then(result => {
      nextBarcodeId++
      console.log('saved Pantry to database')
...
router.post('/updateSpecificPantry', (req, res, next) => {
  Pantry.find({ ourId: req.body.ourID }) // Always returns an array
    .then(pantry => {
      ...
      specificPantry.name = req.body.name;
...
router.post('/updateSpecificHome', (req, res, next) => {
  Pantry.find({ ourId: req.body.ourID }) // Always returns an array
    .then(pantry => {
      let specificPantry = pantry[0]; // pick the first match
      specificPantry.amount = req.body.amount;
